...
Toute la difficulté est donc de calculer le terme $R(s, X_n)$. Celui-ci permet de compter le nombre de sous-séquences qui ont déjà été énumérées dans les rangs précédents. Pour l'exemple qui a été utilisé, on se rend compte que de l'étape 3 à 4, on a pas réutilisé les séquences $<>$ et $<h>$, car celles-ci auraient générées $<o>$ et $<h,o>$ qui sont déjà présentes. En fait, ce facteur correcteur $R(s, X_n)$ correspond aux sous-séquences du rang n-1 qui sont déjà dans les solutions si on leur concatène $X_n$. Autrement dit, ce sont **les solutions du rang n-1 finissant par $X_n$, auxquelles on retranche $X_n$**, càd $\phi(x^{l-1})$, avec l l'indice du dernier itemset où est apparu $X_n$. Ceci n'est pas une vrai démonstration, mais ça devrait donner l'intuition. 
